package phase_zinc.workers

import io.bazel.rulesscala.worker.{GenericWorker, Processor};

import phase_zinc.common.logger.AnnexLogger
import phase_zinc.common.logger.AnnexScalaInstance
import phase_zinc.common.logger.CommonArguments
import phase_zinc.common.logger.FileUtil
import phase_zinc.common.logger.LoggedReporter
...
class ZincProcessor extends Processor {
  ...
  def processRequest(args: java.util.List[String]) {
    ...
    val namespace = parser.parseArgsOrFail(args.asScala.toArray)

    val usePersistence: Boolean = namespace.getBoolean("use_persistence") match {
      case p: java.lang.Boolean => p
      case _                    => true
    }

    val persistenceDir = pathFrom(namespace, "persistence_dir")

    val depsCache = pathFrom(namespace, "extracted_file_cache")
